...
并遵循 [Flutter 架构指南][Flutter Architecture guidelines]。
...
`UserProfileRepository` 将负责在必要时从一个转换为另一个。
...
作为第一种方法，
你可以通过在用户离线或网络调用失败时使用后备机制来实现离线支持。
